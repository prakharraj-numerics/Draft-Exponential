...
/* after r: exact exp(rd) then exact 2^(kn/128) */mpfr_set_d(R,rd,MPFR_RNDN);mpfr_exp(V,R,MPFR_RNDN);mpfr_set_si(T,kn,MPFR_RNDN);mpfr_div_ui(T,T,128,MPFR_RNDN);mpfr_mul(T,T,L,MPFR_RNDN);mpfr_exp(T,T,MPFR_RNDN);mpfr_mul(V,V,T,MPFR_RNDN);hit(&c[0],rnd(V),ref);
/* after Q: exact 1+r*h^2 and exact table/scale factor */mpfr_set_d(Q,h,MPFR_RNDN);mpfr_mul(S,Q,Q,MPFR_RNDN);mpfr_set_d(R,rd,MPFR_RNDN);mpfr_mul(V,R,S,MPFR_RNDN);mpfr_add_ui(V,V,1,MPFR_RNDN);mpfr_set_si(T,kn,MPFR_RNDN);mpfr_div_ui(T,T,128,MPFR_RNDN);mpfr_mul(T,T,L,MPFR_RNDN);mpfr_exp(T,T,MPFR_RNDN);mpfr_mul(V,V,T,MPFR_RNDN);hit(&c[1],rnd(V),ref);
/* after ER: exact multiply er * exact 2^(kn/128) */mpfr_set_d(V,er,MPFR_RNDN);mpfr_set_si(T,kn,MPFR_RNDN);mpfr_div_ui(T,T,128,MPFR_RNDN);mpfr_mul(T,T,L,MPFR_RNDN);mpfr_exp(T,T,MPFR_RNDN);mpfr_mul(V,V,T,MPFR_RNDN);hit(&c[2],rnd(V),ref);
