...
static double bo(fn_t f,int n){long reps=reps_for(n);for(int i=0;i<500;i++)f(y,x,n);double a=sec();for(long i=0;i<reps;i++)f(y,x,n);double b=sec();return(b-a)*1e9/((double)reps*n);}
static double bi(int n){long reps=reps_for(n);for(int i=0;i<500;i++)vmdExp(n,x,z,VML_HA);double a=sec();for(long i=0;i<reps;i++)vmdExp(n,x,z,VML_HA);double b=sec();return(b-a)*1e9/((double)reps*n);}
...
int main(void){
 ...
 for(unsigned b=0;b<sizeof(bs)/sizeof(bs[0]);b++){
   int n=bs[b]; double it=bi(n); printf("BATCH n=%d INTEL_HA=%.6f\n",n,it);
   for(unsigned i=0;i<sizeof(v)/sizeof(v[0]);i++){double t=bo(v[i].f,n);printf("BATCH n=%d %-11s=%.6f intel_over=%.4fx\n",n,v[i].n,t,it/t);} }
 return 0;
}
