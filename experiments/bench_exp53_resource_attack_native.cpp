...
static size_t split_for(size_t n){return ((n/32)/2)*32;}

/* Exact frozen scheduler shape, parameterized only so the same scheduler can
   isolate kernel changes. */
...
/* Attack scheduler: single-writer release store (no locked fetch_add), direct
   common-path kernel call (no fn_ traffic), and relaxed compare polling followed
   by an acquire fence. On x86 this allows cmp-memory polling. */
...
public:
    AttackDispatcherCounted():generation_(0),completed_(0),stop_(false),out_(nullptr),in_(nullptr),n2_(0){pin(0);helper_=std::thread([this]{loop();});while(!ready_.load(std::memory_order_acquire))_mm_pause();}
    ~AttackDispatcherCounted(){stop_.store(true,std::memory_order_relaxed);generation_.store(++seq_,std::memory_order_release);if(helper_.joinable())helper_.join();}
    void run(double*out,const double*in,size_t n){size_t full=n/32;if(full<2){exp53_attack_full(out,in,n);return;}size_t split=(full/2)*32;out_=out+split;in_=in+split;n2_=n-split;uint64_t g=++seq_;generation_.store(g,std::memory_order_release);exp53_attack_full(out,in,split);uint64_t s=0;while(completed_.load(std::memory_order_relaxed)!=g){_mm_pause();++s;}std::atomic_thread_fence(std::memory_order_acquire);caller_+=s;++calls_;}
    uint64_t helper()const{return helper_;}uint64_t caller()const{return caller_;}uint64_t calls()const{return calls_;}
private:
    void loop(){pin(2);ready_.store(true,std::memory_order_release);uint64_t seen=generation_.load(std::memory_order_relaxed);for(;;){uint64_t s=0;while(generation_.load(std::memory_order_relaxed)==seen){_mm_pause();++s;}std::atomic_thread_fence(std::memory_order_acquire);helper_+=s;uint64_t g=generation_.load(std::memory_order_relaxed);seen=g;if(stop_.load(std::memory_order_relaxed))return;exp53_attack_full(out_,in_,n2_);completed_.store(g,std::memory_order_release);}}
    std::thread helper_;alignas(64)std::atomic<uint64_t>generation_,completed_;alignas(64)std::atomic<bool>ready_{false};std::atomic<bool>stop_;uint64_t seq_=0;double*out_;const double*in_;size_t n2_;uint64_t helper_=0,caller_=0,calls_=0;
};
