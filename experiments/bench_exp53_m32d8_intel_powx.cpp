...
#include "exp53_m32d8_intel_powx_candidate.hpp"

using fn_t=void(*)(double*,const double*,size_t);
static volatile double g_sink=0.0;
extern "C" __attribute__((noinline)) void exp53_powx_profile_start(){asm volatile("":::"memory");}
extern "C" __attribute__((noinline)) void exp53_powx_profile_stop(){asm volatile("":::"memory");}
...
static void run_raw(double*o,const double*x,size_t n){exp53_power_family_raw::m32d8(o,x,n);}
static void run_intelpowx(double*o,const double*x,size_t n){exp53_m32d8_intel_powx::intel_powx(o,x,n);}
...
static fn_t choose(const std::string&s){if(s=="raw")return run_raw;if(s=="intelpowx")return run_intelpowx;if(s=="intel")return run_intel;if(s=="noop")return run_noop;return nullptr;}
...
static int accuracy(const std::string&stack){
 fn_t fn=choose(stack);if(!fn||stack=="noop")return 2;constexpr size_t N=9600;
 double*x=(double*)al64(N*8),*y=(double*)al64(N*8),*intel=(double*)al64(N*8);if(!x||!y||!intel)return 3;
 for(size_t i=0;i<N;i++){double a;if(i==0)a=0.0;else if(i==1)a=std::nextafter(1.0,0.0);else if(i==2)a=1.0;else if(i==3)a=0x1p-20;else{double q=u01(UINT64_C(0xd1b54a32d192ed03)+i*UINT64_C(0x9e3779b97f4a7c15));a=0x1p-20+(1.0-0x1p-20)*q;}x[i]=(i&1)?-a:a;}
 fn(y,x,N);run_intel(intel,x,N);mpfr_t mx,mr;mpfr_init2(mx,256);mpfr_init2(mr,256);uint64_t maxm=0,maxi=0;size_t gt1=0,gt2=0;double wx=0,wy=0,wr=0;
 for(size_t i=0;i<N;i++){mpfr_set_d(mx,x[i],MPFR_RNDN);mpfr_exp(mr,mx,MPFR_RNDN);double ref=mpfr_get_d(mr,MPFR_RNDN);uint64_t dm=ulpdist(y[i],ref),di=ulpdist(y[i],intel[i]);if(dm>maxm){maxm=dm;wx=x[i];wy=y[i];wr=ref;}if(di>maxi)maxi=di;if(dm>1)gt1++;if(dm>2)gt2++;}
 std::printf("POWX_ACCURACY stack=%s cases=%zu reference=MPFR256 maxulp_vs_mpfr=%llu count_gt1_mpfr=%zu count_gt2_mpfr=%zu maxulp_vs_intel=%llu worst_x=%.17g worst_y=%.17g worst_ref=%.17g\n",stack.c_str(),N,(unsigned long long)maxm,gt1,gt2,(unsigned long long)maxi,wx,wy,wr);
 mpfr_clear(mx);mpfr_clear(mr);std::free(intel);std::free(y);std::free(x);return 0;
}

static int native(const std::string&stack,size_t n,bool neg){fn_t fn=choose(stack);if(!fn)return 2;double*x=(double*)al64(n*8),*y=(double*)al64(n*8),*ref=(double*)al64(n*8);if(!x||!y||!ref)return 3;fill(x,n,neg);run_intel(ref,x,n);fn(y,x,n);uint64_t mx=0;for(size_t i=0;i<n;i++)mx=std::max(mx,ulpdist(y[i],ref[i]));for(int w=0;w<5;w++)fn(y,x,n);size_t reps=reps_for(n);double wall[7],cpu[7];for(int t=0;t<7;t++){double w0=ns(CLOCK_MONOTONIC_RAW),c0=ns(CLOCK_PROCESS_CPUTIME_ID);for(size_t r=0;r<reps;r++)fn(y,x,n);double c1=ns(CLOCK_PROCESS_CPUTIME_ID),w1=ns(CLOCK_MONOTONIC_RAW);wall[t]=(w1-w0)/((double)reps*n);cpu[t]=(c1-c0)/((double)reps*n);g_sink+=y[(n*7u/11u+t)%n];}std::sort(wall,wall+7);std::sort(cpu,cpu+7);std::printf("POWX_NATIVE stack=%s domain=%s n=%zu reps=%zu wall_ns_el=%.9f cpu_ns_el=%.9f maxulp_vs_intel=%llu sink=%.17g\n",stack.c_str(),neg?"neg":"pos",n,reps,wall[3],cpu[3],(unsigned long long)mx,(double)g_sink);std::free(ref);std::free(y);std::free(x);return 0;}

static int sde(const std::string&stack,size_t n,bool neg,size_t calls){fn_t fn=choose(stack);if(!fn)return 2;double*x=(double*)al64(n*8),*y=(double*)al64(n*8);if(!x||!y)return 3;fill(x,n,neg);fn(y,x,n);exp53_powx_profile_start();for(size_t r=0;r<calls;r++)fn(y,x,n);exp53_powx_profile_stop();if(stack!="noop")g_sink+=y[(n*5u/13u)%n];std::printf("POWX_SDE stack=%s domain=%s n=%zu calls=%zu sink=%.17g\n",stack.c_str(),neg?"neg":"pos",n,calls,(double)g_sink);std::free(y);std::free(x);return 0;}
