...
#include "experiments/exp53_park_inactive_helpers_candidate.hpp"
...
static void fill_domain(double* x,size_t n,int domain){
    ...
    const uint64_t seed=UINT64_C(0x8f3a21d75c6e490b)^(static_cast<uint64_t>(n)<<17)^(static_cast<uint64_t>(domain)<<57);
...
static void prime_current(Exp53BatchProductionExecutor& ex,int domain){
    Aligned in(5000),out(5000); fill_domain(in.p,5000,domain);
    for(int k=0;k<3;++k) ex.run(out.p,in.p,2000,2);
    for(int k=0;k<3;++k) ex.run(out.p,in.p,5000,2);
}
static void prime_candidate(Exp53ParkInactiveHelpersCandidate& ex,int domain){
    Aligned in(5000),out(5000); fill_domain(in.p,5000,domain);
    for(int k=0;k<3;++k) ex.run(out.p,in.p,2000,2);
    for(int k=0;k<3;++k) ex.run(out.p,in.p,5000,2);
}

extern "C" __attribute__((noinline,used)) void exp53_park_profile_start(){ asm volatile("":::"memory"); }
extern "C" __attribute__((noinline,used)) void exp53_park_profile_stop(){ asm volatile("":::"memory"); }
...
static int run_native(const std::string& stack,size_t n,int domain){
    ...
    std::unique_ptr<Exp53BatchProductionExecutor> cur;
    std::unique_ptr<Exp53ParkInactiveHelpersCandidate> cand;
    if(stack=="current"){ cur=std::make_unique<Exp53BatchProductionExecutor>(2); prime_current(*cur,domain); }
    if(stack=="candidate"){ cand=std::make_unique<Exp53ParkInactiveHelpersCandidate>(2); prime_candidate(*cand,domain); }
    auto invoke=[&]{
        if(stack=="intel") vmdExp((MKL_INT)n,in.p,out.p,VML_HA);
        else if(stack=="current") cur->run(out.p,in.p,n,2);
        else cand->run(out.p,in.p,n,2);
    };
    invoke();
    ...
    int hwe=-1,hwa=-1,ce=-1,ca=-1;
    if(cand){ hwe=cand->highway_exists(); hwa=cand->highway_active(); ce=cand->custom_exists(); ca=cand->custom_active(); }
    std::cout<<std::fixed<<std::setprecision(9)
             ...
             <<" hwy_exists="<<hwe<<" hwy_active="<<hwa<<" custom_exists="<<ce<<" custom_active="<<ca<<"\n";
...
static int run_sde(const std::string& stack,size_t n,int domain,size_t calls){
    ...
    std::unique_ptr<Exp53BatchProductionExecutor> cur;
    std::unique_ptr<Exp53ParkInactiveHelpersCandidate> cand;
    if(stack=="current"){ cur=std::make_unique<Exp53BatchProductionExecutor>(2); prime_current(*cur,domain); for(int i=0;i<3;++i)cur->run(out.p,in.p,n,2); if(maxulp_vs(out.p,ref.p,n)>2)return 4; }
    if(stack=="candidate"){ cand=std::make_unique<Exp53ParkInactiveHelpersCandidate>(2); prime_candidate(*cand,domain); for(int i=0;i<3;++i)cand->run(out.p,in.p,n,2); if(maxulp_vs(out.p,ref.p,n)>2)return 4; }
    if(stack=="intel") for(int i=0;i<3;++i)vmdExp((MKL_INT)n,in.p,out.p,VML_HA);
    exp53_park_profile_start();
    if(stack=="current") for(size_t k=0;k<calls;++k)cur->run(out.p,in.p,n,2);
    else if(stack=="candidate") for(size_t k=0;k<calls;++k)cand->run(out.p,in.p,n,2);
    else if(stack=="intel") for(size_t k=0;k<calls;++k)vmdExp((MKL_INT)n,in.p,out.p,VML_HA);
    else for(size_t k=0;k<calls;++k) asm volatile(""::"r"(k):"memory");
    exp53_park_profile_stop();
...
int main(int argc,char**argv){
    ...
    if(argc<5){ std::cerr<<"usage: native|sde candidate|current|intel|noop N unit|mid [calls]\n"; return 2; }
    ...
    if(mode=="native"){
        if(stack!="candidate"&&stack!="current"&&stack!="intel")return 2;
        return run_native(stack,n,domain);
    }
    if(mode=="sde"){
        if(argc!=6||(stack!="candidate"&&stack!="current"&&stack!="intel"&&stack!="noop"))return 2;
