...
template<class F> static double tim(F&&f,size_t n,size_t calls,int samples){for(int w=0;w<5;++w)f();std::vector<double>s;for(int k=0;k<samples;++k){auto a=std::chrono::steady_clock::now();for(size_t c=0;c<calls;++c)f();auto b=std::chrono::steady_clock::now();s.push_back(std::chrono::duration<double,std::nano>(b-a).count()/((double)calls*n));}return med(s);}

int main(){
 constexpr size_t M=3000; Buf in(M),ref(M),out(M); Exp53Custom2ConservativeSafeCandidate c; volatile double sink=0;
 std::cout<<std::fixed<<std::setprecision(9);
 std::cout<<"CUSTOM2_CONSERVATIVE_SAFE sizes=100..3000 step=50 split_boundary=32 bitwise_gate=required\n";
 for(int d=0;d<2;++d){const char*dn=d?"mid":"unit";for(size_t n=100;n<=3000;n+=50){
  fill(in.p,n,d); exp53_n2_vmstyle_u4_0381_frozen(ref.p,in.p,n); std::memset(out.p,0,n*sizeof(double)); c.run(out.p,in.p,n);
  int bd=std::memcmp(out.p,ref.p,n*sizeof(double))!=0; if(bd){std::cout<<"FAIL domain="<<dn<<" n="<<n<<" bitdiff=1\n";return 9;}
  size_t calls=calls_for(n); auto fc=[&]{exp53_n2_vmstyle_u4_0381_frozen(out.p,in.p,n);}; auto cc=[&]{c.run(out.p,in.p,n);}; auto ic=[&]{vmdExp((MKL_INT)n,in.p,out.p,VML_HA);};
  double f=tim(fc,n,calls,7), x=tim(cc,n,calls,7), i=tim(ic,n,calls,7); sink+=out.p[(n*7u+d)%n]*0x1p-1022;
  std::cout<<"FINAL domain="<<dn<<" n="<<n<<" share="<<Exp53Custom2ConservativeSafeCandidate::conservative_share(n)<<" custom_ns="<<x<<" frozen_ns="<<f<<" intel_ns="<<i<<" intel_over_custom="<<(i/x)<<" frozen_over_custom="<<(f/x)<<" bitdiff=0\n";
 }}
