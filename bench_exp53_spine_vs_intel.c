...
void exp53_spine_v128_frozen(double*,const double*,size_t);
...
static void inputs(void){
  ...
  for(int i=0;i<1600;i++)x[i]=0x1p-20+uni(&s)*(1-0x1p-20);
  for(int i=1600;i<3200;i++)x[i]=-(0x1p-20+uni(&s)*(1-0x1p-20));
  for(int i=3200;i<4800;i++)x[i]=1+uni(&s)*99;
  for(int i=4800;i<6400;i++)x[i]=-(1+uni(&s)*99);
}

static void acc(fn_t f){
  f(y,x,N);uint64_t mx=0;int g1=0,g2=0,g4=0;long double mr=0;
  for(int i=0;i<N;i++){
    double r=exp(x[i]);uint64_t d=D(y[i],r);if(d>mx)mx=d;
    if(d>1)g1++;if(d>2)g2++;if(d>4)g4++;
    long double e=fabsl(((long double)y[i]-r)/r);if(e>mr)mr=e;
  }
  printf("ACC v128_frozen maxulp=%llu gt1=%d gt2=%d gt4=%d maxrel=%.3Le\n",(unsigned long long)mx,g1,g2,g4,mr);
}
...
int main(void){
  ...
  inputs();acc(exp53_spine_v128_frozen);
  double is=bi(0,HALF),iw=bi(HALF,HALF),ia=bi(0,N);
  double os=bo(exp53_spine_v128_frozen,0,HALF),ow=bo(exp53_spine_v128_frozen,HALF,HALF),oa=bo(exp53_spine_v128_frozen,0,N);
  printf("INTEL small=%.6f wide=%.6f all=%.6f\n",is,iw,ia);
  printf("V128_FROZEN small=%.6f wide=%.6f all=%.6f intel_over_all=%.4fx\n",os,ow,oa,ia/oa);
  return 0;
}
