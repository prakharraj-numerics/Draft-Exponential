...
static void acc(const char*n,fn_t f){f(y,x,N);uint64_t mx=0;int g1=0,g2=0,g4=0;long double mr=0;for(int i=0;i<N;i++){double r=exp(x[i]);uint64_t d=D(y[i],r);if(d>mx)mx=d;if(d>1)g1++;if(d>2)g2++;if(d>4)g4++;long double e=fabsl(((long double)y[i]-r)/r);if(e>mr)mr=e;}printf("ACC %-10s maxulp=%llu gt1=%d gt2=%d gt4=%d maxrel=%.3Le\n",n,(unsigned long long)mx,g1,g2,g4,mr);}
...
int main(void){setenv("MKL_NUM_THREADS","1",1);setenv("OMP_NUM_THREADS","1",1);mkl_set_num_threads_local(1);inputs();acc("u4_frozen",exp53_spine_v128_u4_frozen);acc("formula6",exp53_spine_v128_u4_formula6);double ha=bi(0,N);printf("INTEL_HA all=%.6f\n",ha);rep("u4_frozen",exp53_spine_v128_u4_frozen,ha);rep("formula6",exp53_spine_v128_u4_formula6,ha);return 0;}
