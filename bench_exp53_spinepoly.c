...
void exp53_spinepoly_mid_u5(double*,const double*,size_t);
void exp53_spinepoly_mid_u6(double*,const double*,size_t);
void exp53_spinepoly_plain_u6(double*,const double*,size_t);
enum{N=12288,HALF=6144,REPS=32000}; static double x[N],y[N],z[N];
...
static double bo(fn_t f,int lo,int n){for(int i=0;i<800;i++)f(y+lo,x+lo,n);double a=sec();for(int i=0;i<REPS;i++)f(y+lo,x+lo,n);double b=sec();return(b-a)*1e9/((double)REPS*n);} static double bi(int lo,int n){for(int i=0;i<800;i++)vmdExp(n,x+lo,z+lo,VML_HA);double a=sec();for(int i=0;i<REPS;i++)vmdExp(n,x+lo,z+lo,VML_HA);double b=sec();return(b-a)*1e9/((double)REPS*n);}
static void rep(const char*n,fn_t f){printf("RES %-12s small=%.6f wide=%.6f all=%.6f\n",n,bo(f,0,HALF),bo(f,HALF,HALF),bo(f,0,N));}
int main(void){setenv("MKL_NUM_THREADS","1",1);setenv("OMP_NUM_THREADS","1",1);mkl_set_num_threads_local(1);inputs();struct{const char*n;fn_t f;}v[]={{"mid_u6_old",exp53_final_mid_u6},{"sp_mid_u4",exp53_spinepoly_mid_u4},{"sp_mid_u5",exp53_spinepoly_mid_u5},{"sp_mid_u6",exp53_spinepoly_mid_u6},{"sp_plain_u6",exp53_spinepoly_plain_u6}};for(unsigned i=0;i<sizeof(v)/sizeof(v[0]);i++)acc(v[i].n,v[i].f);printf("INTEL small=%.6f wide=%.6f all=%.6f\n",bi(0,HALF),bi(HALF,HALF),bi(0,N));for(unsigned i=0;i<sizeof(v)/sizeof(v[0]);i++)rep(v[i].n,v[i].f);return 0;}
