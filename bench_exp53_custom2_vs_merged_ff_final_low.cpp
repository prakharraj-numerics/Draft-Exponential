...
static int calls_for(size_t n){uint64_t c=5000000ULL/n;if(c<2048)c=2048;if(c>100000)c=100000;return(int)c;}
static size_t stream_slots(size_t n){const size_t target=64ULL*1024ULL*1024ULL,one=n*8;size_t s=(target+one-1)/one;if(s<8)s=8;if(s>4096)s=4096;return s;}
...
template<class E>static uint64_t accuracy(E&ex,bool nt){const size_t n=3000;double*in=(double*)xalloc(n*8),*r=(double*)xalloc(n*8),*o=(double*)xalloc(n*8);for(size_t i=0;i<n;i++)in[i]=rd();exp53_n2_vmstyle_u4_0381_frozen(r,in,n);if(nt)ex.run_streaming_write_once(o,in,n);else ex.run(o,in,n);uint64_t d=0;for(size_t i=0;i<n;i++)d+=bits(r[i])!=bits(o[i]);std::free(in);std::free(r);std::free(o);return d;}
...
template<class E>static int run(const char*name,E&ex){printf("STACK=%s default=temporal rare=streaming_nt\n",name);printf("ACCURACY stack=%s default_bitdiff=%llu streaming_bitdiff=%llu\n",name,(unsigned long long)accuracy(ex,false),(unsigned long long)accuracy(ex,true));const size_t ns[]={50,100,250,500,750,1000,3000};for(size_t n:ns){rs=0x9e3779b97f4a7c15ULL^n;double*in=(double*)xalloc(n*8),*out=(double*)xalloc(n*8);for(size_t i=0;i<n;i++)in[i]=rd();Ring ring(stream_slots(n),n);int c=calls_for(n);for(int w=0;w<128;w++)ex.run(out,in,n);for(int w=0;w<32;w++)ex.run_streaming_write_once(ring.at(w),in,n);std::vector<double>h,s;for(int q=0;q<25;q++)h.push_back(hot(ex,out,in,n,c));for(int q=0;q<25;q++)s.push_back(stream(ex,ring,in,n,c,q*13));printf("METHOD stack=%s n=%zu calls=%d slots=%zu hot_ns=%.9f stream_nt_ns=%.9f\n",name,n,c,ring.slots(),median(h),median(s));std::free(out);std::free(in);}return 0;}
