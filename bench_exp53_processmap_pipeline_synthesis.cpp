...
void exp53_pm_u3(double*,const double*,size_t);
void exp53_pm_u4(double*,const double*,size_t);
void exp53_pm_u5(double*,const double*,size_t);
void exp53_pm_u6(double*,const double*,size_t);
void exp53_pm_u4_stagger(double*,const double*,size_t);
}
...
int main(int argc,char**argv){
 ...
 if(st=="u3")fn=exp53_pm_u3; else if(st=="u4")fn=exp53_pm_u4; else if(st=="u5")fn=exp53_pm_u5; else if(st=="u6")fn=exp53_pm_u6; else if(st=="stagger")fn=exp53_pm_u4_stagger; else if(st=="frozen")fn=exp53_n2_vmstyle_u4_0381_frozen; else if(st!="intel")return 3;
 std::vector<int> ns;for(int n=100;n<=1400;n+=50)ns.push_back(n);if(std::getenv("SWEEP_REVERSE"))std::reverse(ns.begin(),ns.end());
 for(const char* dom:{"unit","mid"})for(int ni:ns){size_t n=(size_t)ni,calls=calls_for(n);std::vector<double> in(n),out(n),ref(n);fill(in,dom);
   if(st!="intel"&&st!="frozen"){exp53_n2_vmstyle_u4_0381_frozen(ref.data(),in.data(),n);fn(out.data(),in.data(),n);std::printf("CHECK stack=%s domain=%s n=%d bitdiff=%zu\n",st.c_str(),dom,ni,bitdiff(out,ref));}
