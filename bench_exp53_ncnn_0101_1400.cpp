...
extern "C" void exp53_ncnn_0101_1400(double*, const double*, size_t, int, int);
...
int main(){
    ...
    std::cout<<std::fixed<<std::setprecision(9)
             <<"NCNN_X86_101_1400 ncnn=60bfa40044efffbba5d4ccae090a037185942a4d formula_only=1\n";
    for(int d=0;d<2;d++){
        ...
        for(size_t n:sizes){
            ...
            exp53_ncnn_0101_1400(out.p,in.p,n,1,50);
            for(size_t i=0;i<n;i++){auto u=ulp(out.p[i],std::exp(in.p[i]));maxu=std::max(maxu,u);if(u>1)gt1++;}
            double one=tim([&]{exp53_ncnn_0101_1400(out.p,in.p,n,1,50);},n,c);

            exp53_ncnn_0101_1400(out.p,in.p,n,2,50);
            for(size_t i=0;i<n;i++){auto u=ulp(out.p[i],std::exp(in.p[i]));maxu=std::max(maxu,u);if(u>1)gt1++;}
            double two=tim([&]{exp53_ncnn_0101_1400(out.p,in.p,n,2,50);},n,c);

            double best=std::min(one,two);
            int threads=(two<one)?2:1;
            ...
            std::cout<<"FINAL domain="<<dn<<" n="<<n
                     <<" ncnn_ns="<<best<<" ncnn_threads="<<threads
                     <<" ncnn_1t_ns="<<one<<" ncnn_2t_ns="<<two
                     <<" frozen_ns="<<fr<<" intel_ns="<<it
                     <<" intel_over_ncnn="<<it/best<<" frozen_over_ncnn="<<fr/best
                     <<" maxulp="<<maxu<<" gt1="<<gt1<<"\n";
        }
    }
}
