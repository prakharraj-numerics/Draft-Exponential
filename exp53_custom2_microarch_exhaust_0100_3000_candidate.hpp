...
public:
 ...
 Exp53Custom2MicroarchExhaustCandidate():gen64_(0),done64_(0),gen128_(0),done128_(0),stop_(false),ready_(false),out2_(nullptr),in2_(nullptr),n2_(0),mode_(BASE),local_gen_(0){pin(0);helper_=std::thread([this]{loop();});while(!ready_.load(std::memory_order_acquire))_mm_pause();}
 ~Exp53Custom2MicroarchExhaustCandidate(){stop_.store(true,std::memory_order_relaxed);uint64_t g=++local_gen_;gen64_.store(g,std::memory_order_release);gen128_.store(g,std::memory_order_release);if(helper_.joinable())helper_.join();}
