...
void exp53_ispc_0101_2000_candidate(double*, const double*, size_t);
void exp53_n2_vmstyle_u4_0381_frozen(double*, const double*, size_t);
void exp53_ispc_debug8_wrapper(const double*, uint64_t*);
void exp53_frozen_debug8_wrapper(const double*, uint64_t*);
...
static void diagnose(const double* in){
  static const char* S[20]={"biased","k","kn","j","q","tb","r1","r2","r","sb","scale","h1","h2","h3","h","s","er","el","ph","y"};
  alignas(64) uint64_t a[160],b[160];
  exp53_ispc_debug8_wrapper(in,a); exp53_frozen_debug8_wrapper(in,b);
  for(int st=0;st<20;st++) for(int lane=0;lane<8;lane++) if(a[st*8+lane]!=b[st*8+lane]){
    std::cout<<"FIRSTDIFF stage="<<S[st]<<" lane="<<lane<<" ispc=0x"<<std::hex<<a[st*8+lane]<<" frozen=0x"<<b[st*8+lane]<<std::dec<<"\n";
    return;
  }
  std::cout<<"FIRSTDIFF stage=none_first8\n";
}
...
int main(){
  ...
  std::cout<<std::fixed<<std::setprecision(9)<<"ISPC_EXP53 sizes=101,150..2000 backend=ISPC-avx512skx-x8 math=frozen no_ispc_exp=1 bitwise_gate=required\n";
  for(int d=0;d<2;d++){
    const char*dn=d?"mid":"unit";
    for(size_t n:sizes){
      fill(in.p,n,d);exp53_n2_vmstyle_u4_0381_frozen(ref.p,in.p,n);
      std::memset(out.p,0,n*8);exp53_ispc_0101_2000_candidate(out.p,in.p,n);
      if(std::memcmp(out.p,ref.p,n*8)){std::cout<<"FAIL domain="<<dn<<" n="<<n<<" bitdiff=1\n";diagnose(in.p);return 9;}
      size_t c=calls(n);
      auto ss=[&]{exp53_ispc_0101_2000_candidate(out.p,in.p,n);};
      auto ff=[&]{exp53_n2_vmstyle_u4_0381_frozen(out.p,in.p,n);};
      auto ii=[&]{vmdExp((MKL_INT)n,in.p,out.p,VML_HA);};
      double st=tim(ss,n,c),ft=tim(ff,n,c),it=tim(ii,n,c);
      std::cout<<"FINAL domain="<<dn<<" n="<<n<<" ispc_ns="<<st<<" frozen_ns="<<ft<<" intel_ns="<<it<<" intel_over_ispc="<<it/st<<" frozen_over_ispc="<<ft/st<<" bitdiff=0\n";
    }
  }
}
