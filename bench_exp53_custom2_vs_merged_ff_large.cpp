...
static int run_method(const char *name, Exec &ex) {
    ...
    const size_t ns[] = {200000,500000,1000000,1500000,2500000};
    for(size_t n:ns) {
        ...
        for(int w=0;w<32;w++) ex.run(hot,in,n);
        for(int w=0;w<8;w++) ex.run_streaming_write_once(ring.at((size_t)w),in,n);
