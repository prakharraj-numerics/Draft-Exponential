...
/* NVC++ is evaluated only on the 32-wide bulk.  The frozen production kernel
   already delegates its remainder to exp53_n2_fused_u4_038_frozen(), so use
   that exact ICX-compiled tail here too.  This removes compiler-libm tail
   differences without changing the NVC++ bulk arithmetic under test. */
static inline void exp53_nvcxx_exact(double *out,const double *in,size_t n){
  const size_t bulk=n&~(size_t)31;
  if(bulk) exp53_nvcxx(out,in,bulk);
...
int main(){
  ...
  for(int d=0;d<2;d++) for(size_t n:sizes){
    ...
    exp53_nvcxx_exact(out.p,in.p,n);
    if(std::memcmp(out.p,ref.p,n*8)){std::cout<<"FAIL domain="<<(d?"mid":"unit")<<" n="<<n<<" bitdiff=1\n";return 9;}
    size_t c=calls(n);
    double nt=tim([&]{exp53_nvcxx_exact(out.p,in.p,n);},n,c);
