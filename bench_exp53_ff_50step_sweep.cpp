// Focused low-batch crossover sweep: frozen FastFlow 2-core vs plain Intel VML_HA.
// Sizes: n=50,100,...,3000. No custom runtime, no NT stores.
#include <algorithm>
#include <chrono>
#include <cstdint>
#include <cstdio>
...
static void run_intel(double *out, const double *in, size_t n) { vmdExp((MKL_INT)n, in, out, VML_HA); }

int main(int argc, char **argv) {
    if (argc != 2) { std::fprintf(stderr, "usage: %s ff|intel\n", argv[0]); return 2; }
    const std::string stack = argv[1];
    if (stack != "ff" && stack != "intel") return 2;
    const bool ff = stack == "ff";
    ...
    Exp53FastFlowFrozenExecutor *pex=nullptr;
    if (ff) pex = new Exp53FastFlowFrozenExecutor(2, exp53_n2_vmstyle_u4_0381_frozen);
    ...
    for (int domain:domains) {
        for (size_t n:sizes) {
            AlignedDoubles in(n),out(n); fill_inputs(in.p,n,domain); const size_t calls=call_count(n);
            auto invoke=[&](){ if(ff) pex->run_static(out.p,in.p,n,2); else run_intel(out.p,in.p,n); };
            ...
            std::cout << "RESULT stack="<<stack<<" domain="<<domain_name(domain)<<" n="<<n<<" calls="<<calls<<" ns="<<median(samples)<<"\n";
        }
    }
    delete pex;
