...
static const double TAB128C[128] = {
  ...
  0x1.ea4afa2a490d9p+0, 0x1.ecf482d8e67f1p+0, 0x1.efa1bee615a27p+0, 0x1.f252b376bba98p+0,
  0x1.f50765b6e4540p+0, 0x1.f7bfdad9cbe14p+0, 0x1.fa7c1819e90d8p+0, 0x1.fd3c22b8f71f1p+0
};

static inline __m512d exp53_v128_u4_formula6_tabcomp_block(__m512d x,const __m512d inv,const __m512d hi,const __m512d mi,const __m512d lo,const __m512d magic,const __m512i mb,const __m512i mask)
{
    ...
    __m512d tab=_mm512_i64gather_pd(j,TAB128C,8);
    return _mm512_mul_pd(_mm512_mul_pd(er,tab),exp2_from_q(q));
}

__attribute__((target("avx512f,avx512dq,fma")))
void exp53_spine_v128_u4_formula6_tabcomp(double *restrict out,const double *restrict in,size_t n)
{
    ...
    for(;i+32<=n;i+=32){
        ...
        __m512d y0=exp53_v128_u4_formula6_tabcomp_block(x0,inv,hi,mi,lo,magic,mb,mask),y1=exp53_v128_u4_formula6_tabcomp_block(x1,inv,hi,mi,lo,magic,mb,mask),y2=exp53_v128_u4_formula6_tabcomp_block(x2,inv,hi,mi,lo,magic,mb,mask),y3=exp53_v128_u4_formula6_tabcomp_block(x3,inv,hi,mi,lo,magic,mb,mask);
        _mm512_storeu_pd(out+i,y0);_mm512_storeu_pd(out+i+8,y1);_mm512_storeu_pd(out+i+16,y2);_mm512_storeu_pd(out+i+24,y3);
    }
    for(;i<n;){if(n-i>=8){_mm512_storeu_pd(out+i,exp53_v128_u4_formula6_tabcomp_block(_mm512_loadu_pd(in+i),inv,hi,mi,lo,magic,mb,mask));i+=8;}else{for(;i<n;++i)out[i]=exp(in[i]);}}
}
