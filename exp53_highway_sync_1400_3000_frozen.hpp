...
/* EXP53 FROZEN Highway synchronized temporal batch path, n=1400..3000.

   This extends the already-frozen 1600..3000 old-sync Highway family down to
   n=1400, using the exact same executor design and mathematical machinery.

   Evidence:
     33499950239 : exact-Xeon old synchronized Highway sweep, n=1200..1600
   ...
   Execution is the proven old-sync family:
     - caller pinned CPU0, permanent helper pinned CPU2
     - generation fetch_add/release handoff retained
     - temporal stores
     - 32-element aligned partition
     - Highway 1.4.0 static AVX-512 backend
     - formula-only Highway vector tail

   Frozen helper-share schedule:
     1400..1439 -> 30%
     1440..1499 -> 32%
     1500..1535 -> 31%
     1536..1599 -> 33%
   ...
   The 1400..1599 buckets are conservative representatives of the exact-Xeon
   best-share sweep; 1600..3000 retains the previously frozen schedule.

   Validation gates in cited runs:
     maxULP vs std::exp = 1
     gt1 = 0
     bulk/serial Highway bitdiff = 0
...
namespace exp53_hwy_1400_frozen_ns {
...
class Exp53HighwaySync1400_3000Frozen {
public:
    Exp53HighwaySync1400_3000Frozen()
        : generation_(0), completed_(0), stop_(false), helper_ready_(false),
          out2_(nullptr), in2_(nullptr), n2_(0) {
        exp53_hwy_1400_frozen_ns::pin(0);
    ...
    Exp53HighwaySync1400_3000Frozen(const Exp53HighwaySync1400_3000Frozen&) = delete;
    Exp53HighwaySync1400_3000Frozen& operator=(const Exp53HighwaySync1400_3000Frozen&) = delete;
    ...
    void run(double* out,const double* in,size_t n) {
        if(n < 1400 || n > 3000) {
            exp53_hwy_1400_frozen_ns::kernel(out,in,n);
            return;
        }
        ...
        exp53_hwy_1400_frozen_ns::kernel(out,in,split);
        while(completed_.load(std::memory_order_acquire)!=g) _mm_pause();
    }

private:
    void helper_loop() {
        exp53_hwy_1400_frozen_ns::pin(2);
        ...
        for(;;) {
            ...
            exp53_hwy_1400_frozen_ns::kernel(out2_,in2_,n2_);
