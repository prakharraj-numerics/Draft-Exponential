...
int main(){
  ...
  auto hx=hn::Load(d,x8); auto hinv=hn::Set(d,N2F_INV128); auto hhi=hn::Set(d,N2F_L128_HI); auto hmi=hn::Set(d,N2F_L128_MI); auto hlo=hn::Set(d,N2F_L128_LO); auto hmagic=hn::Set(d,N2F_MAGIC); auto hone=hn::Set(d,1.0); auto hq1=hn::Set(d,N2F_Q1); auto hq2=hn::Set(d,N2F_Q2); auto hq3=hn::Set(d,N2F_Q3); auto hq4=hn::Set(d,N2F_Q4);
  auto hmb=hn::Set(di,(int64_t)N2F_MAGIC_BITS); auto hmask=hn::Set(di,127);
  auto hb=hn::MulAdd(hx,hinv,hmagic); auto hk=hn::Sub(hb,hmagic); auto hkn=hn::Sub(hn::BitCast(di,hb),hmb); auto hj=hn::And(hkn,hmask); auto hqq=hn::ShiftRight<7>(hkn); auto htb=hn::GatherIndex(di,(const int64_t*)N2_FROZEN_TAB128,hj);
  auto hr=hn::NegMulAdd(hk,hhi,hx);hr=hn::NegMulAdd(hk,hmi,hr);hr=hn::NegMulAdd(hk,hlo,hr);auto hh=hn::MulAdd(hq4,hr,hq3);hh=hn::MulAdd(hh,hr,hq2);hh=hn::MulAdd(hh,hr,hq1);hh=hn::MulAdd(hh,hr,hone);auto hs=hn::Mul(hh,hh);auto her=hn::MulAdd(hr,hs,hone);auto hel=hn::MulAdd(hr,hs,hn::Sub(hone,her));auto hsb=hn::Add(htb,hn::ShiftLeft<52>(hqq));auto hsc=hn::BitCast(d,hsb);auto hph=hn::Mul(her,hsc);auto hy=hn::MulAdd(hel,hsc,hph);
