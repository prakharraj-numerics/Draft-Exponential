...
int main(){B in(2000),ref(2000),out(2000);const size_t sizes[]={100,125,150,200,250,300,400,500,600,750,900,1000,1100,1250,1400,1500,1600,1700,1750,1800,1850,1900,1950,2000};std::cout<<std::fixed<<std::setprecision(9)<<"HIGHWAY_U2Z_SERIAL cpu=0 granularity=16 bitdiff=reported_nonfatal\n";for(int d=0;d<2;d++){const char*dn=d?"mid":"unit";for(size_t n:sizes){fill(in.p,n,d);exp53_n2_vmstyle_u4_0381_frozen(ref.p,in.p,n);exp53_highway_u2z_0100_2000_candidate(out.p,in.p,n);size_t bad_count=0,first_bad=n;for(size_t i=0;i<n;i++){if(std::memcmp(out.p+i,ref.p+i,sizeof(double))!=0){if(first_bad==n)first_bad=i;++bad_count;}}size_t c=calls(n);double ser=tim([&]{exp53_highway_u2z_0100_2000_candidate(out.p,in.p,n);},n,c);double fro=tim([&]{exp53_n2_vmstyle_u4_0381_frozen(out.p,in.p,n);},n,c);double intel=tim([&]{vmdExp((MKL_INT)n,in.p,out.p,VML_HA);},n,c);std::cout<<"FINAL domain="<<dn<<" n="<<n<<" highway_u2z_ns="<<ser<<" frozen_ns="<<fro<<" intel_ns="<<intel<<" intel_over_highway_u2z="<<intel/ser<<" frozen_over_highway_u2z="<<fro/ser<<" bitdiff_count="<<bad_count; if(bad_count)std::cout<<" first_bad="<<first_bad; std::cout<<"\n";}}return 0;}
