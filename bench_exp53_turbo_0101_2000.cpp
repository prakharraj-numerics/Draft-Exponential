...
void exp53_turbo_0101_2000_candidate(double*, const double*, size_t);
...
int main(){
  ...
  std::cout<<std::fixed<<std::setprecision(9)<<"TURBO_EXP53 sizes=101,150..2000 backend=Turbo-AVX512 math=frozen no_turbo_exp=1 turbo_commit=9b212dedfaa4d4e08a854776e1ddaca7d746444c bitwise_gate=required\n";
  for(int d=0;d<2;d++){
    const char*dn=d?"mid":"unit";
    for(size_t n:sizes){
      fill(in.p,n,d);exp53_n2_vmstyle_u4_0381_frozen(ref.p,in.p,n);
      std::memset(out.p,0,n*8);exp53_turbo_0101_2000_candidate(out.p,in.p,n);
      if(std::memcmp(out.p,ref.p,n*8)){std::cout<<"FAIL domain="<<dn<<" n="<<n<<" bitdiff=1\n";return 9;}
      size_t c=calls(n);
      auto tt=[&]{exp53_turbo_0101_2000_candidate(out.p,in.p,n);};
      auto ff=[&]{exp53_n2_vmstyle_u4_0381_frozen(out.p,in.p,n);};
      auto ii=[&]{vmdExp((MKL_INT)n,in.p,out.p,VML_HA);};
      double t=tim(tt,n,c),f=tim(ff,n,c),it=tim(ii,n,c);
      std::cout<<"FINAL domain="<<dn<<" n="<<n<<" turbo_ns="<<t<<" frozen_ns="<<f<<" intel_ns="<<it<<" intel_over_turbo="<<it/t<<" frozen_over_turbo="<<f/t<<" bitdiff=0\n";
    }
  }
}
