...
/* EXP53 candidate: purpose-built permanent 2-core dispatcher.

   Goal: minimize fixed dispatch/synchronization cost for small/medium batches.
   ...
   Execution model on the validated 2-physical-core Xeon runner:
     ...
     - no queue, no scheduler, no work stealing, no task allocation

   Store policy matches the merged production semantics:
     - run(): temporal frozen kernel (default/common case)
     - run_streaming_write_once(): NT kernel (explicit rare case)

   This is a CANDIDATE. It does not modify any frozen survivor.
*/
...
class Exp53CustomPermanent2CoreCandidate {
public:
    using fn_t = void (*)(double*, const double*, size_t);

    Exp53CustomPermanent2CoreCandidate()
        : generation_(0), completed_(0), stop_(false),
          out_(nullptr), in_(nullptr), n2_(0), fn_(nullptr) {
        // Caller is an actual worker in this runtime, so pinning it to one
        // physical core does not create a third-thread contention problem.
    ...
    Exp53CustomPermanent2CoreCandidate(const Exp53CustomPermanent2CoreCandidate&) = delete;
    Exp53CustomPermanent2CoreCandidate& operator=(const Exp53CustomPermanent2CoreCandidate&) = delete;
...
private:
    ...
    void helper_loop() {
        pin_current_thread(2); // CPU 2 = physical core 1 on validated runner.
    ...
    void run_with(fn_t fn, double *out, const double *in, size_t n) {
        ...
        // Split only on 32-value boundaries so each half preserves the exact
        // frozen u4 vector grouping. Caller gets [0,split), helper [split,n).
    ...
    // Published before generation_ release; consumed after acquire.
    double *out_;
    const double *in_;
    size_t n2_;
    fn_t fn_;
